		...
		<div id="browser-settings-section" style={{ marginBottom: 20 }}>
			...
			<CollapsibleContent isOpen={isSubSettingsOpen}>
				...
				<div style={{ marginBottom: 0 }}>
					...
					{isBundled ? "（未在您的机器上检测到）" : detectedChromePath ? ` (${detectedChromePath})` : ""}。您
					可以在下方指定自定义路径。使用远程浏览器连接需要以调试模式启动 Chrome
					{localBrowserSettings.remoteBrowserEnabled ? (
						<>
							{" "}
							手动（<code>--remote-debugging-port=9222</code>）或使用下方按钮。输入主机
							地址或留空以自动发现。
						</>
					) : (
						"。"
					...
					<div style={{ marginBottom: 8, marginTop: 8 }}>
						<label htmlFor="chrome-executable-path" style={{ fontWeight: "500", display: "block", marginBottom: 5 }}>
							Chrome Executable Path (Optional)Path (Optional)
						</label>
						<VSCodeTextField
