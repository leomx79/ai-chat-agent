	...
	*/
	...
	async handleWebviewMessage(message: WebviewMessage) {
		if (message.type === "discussion") {
			await this.handleDiscussionMessage(message)
			return
		}
		switch (message.type) {
	...
	private async handleDiscussionMessage(message: WebviewMessage) {
		switch (message.discussionAction) {
			case "create": {
				// Abort existing discussion
				...
				this.discussionManager = new DiscussionManager(
					...
					(item) => {
						this.discussionStreamItems.push(item)
						this.postMessageToWebview({ type: "discussion", discussionStreamItem: item })
					},
					(error) => {
			...
			case "chat": {
				if (this.discussionManager && message.discussionText) {
					await this.discussionManager.handleChatMessage(message.discussionText)
				}
				break
			}
			case "stop": {
			...
			case "generateProposal": {
				if (this.discussionManager) {
					await this.discussionManager.generateProposal()
				}
				break
			}
			case "approveProposal": {
				if (this.discussionManager) {
					await this.discussionManager.applyProposal()
				}
				break
			}
			case "rejectProposal": {
				if (this.discussionManager) {
					this.discussionManager.rejectProposal()
				}
				break
			}
		}
